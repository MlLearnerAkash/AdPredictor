...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        leave=False,):
        #print(anomaly_score)
        # print("segmentation",np.array(segmentation).mean())
        # print("anomaly score",anomaly_score)

        # if anomaly_score <threshold:
        max_ind = np.unravel_index(segmentation.argmax(), segmentation.shape)
        max_val =segmentation[max_ind]

        l_ano_score.append(max_val)
        


        # seg =np.where((segmentation<(max_val+0.02)) &(segmentation>(max_val-0.02)))
        # print('mean of max values>>> ',segmentation[seg].mean())
        ...
        f, axes = plt.subplots(1, 2 + int(masks_provided))#+ int(masks_provided)
        axes[0].imshow(image.transpose(1, 2, 0))
        axes[1].imshow(mask.transpose(1, 2, 0))
        axes[2].text(1, 10, f'anomaly score:{anomaly_score}', style='italic')
        axes[2].text(30, 70, f'anomaly_coordinates:{max_ind}', style='italic')
        axes[2].text(60, 140, f'max_val:{max_val}', style='italic')
        ...
        f.tight_layout()
        if "bmp" in (savename):
            savename =savename.replace("bmp","png")
        else:
            pass
        f.savefig(savename)
        # plt.close()
        # cv2.imwrite(savename,segmentation)
        # print("segmentation image shape:",segmentation.min(), segmentation.max())
        # im=PIL.Image.fromarray(segmentation*1000)
        # im=im.convert('RGB')
        # im.save(savename)
    print(l_ano_score)
