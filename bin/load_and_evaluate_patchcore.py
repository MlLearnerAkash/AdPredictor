...
def run(methods, results_path, gpu, seed, save_segmentation_images):
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        with device_context:
            ...
            # scaled = scaler.fit_transform(segmentations)
            scores = (scores - min_scores) / (max_scores - min_scores)
            print("final scores: ",scores)
            scores = np.mean(scores, axis=0) #change:mean-->max
            
            

            # for i, data in enumerate(dataloaders['testing']):
            #     print(data["image"])
            ...
            # print(segmentations[0].sum(),segmentations[10].sum())
            # for i in range(0,16):
            #     print(segmentations[i].sum())
            anomaly_labels = [
            ...
            if save_segmentation_images:
                ...
                def image_transform(image):
                    in_std = np.array(
                        (0.3879 ,0.3879, 0.3879) #std values
                    ).reshape(-1, 1, 1)
                    in_mean = np.array(
                        (0.5539, 0.5530, 0.5530) #dataloaders["testing"].dataset.transform_mean
                    ).reshape(-1, 1, 1)
            ...
            #print("Higher scores=> higher probablity of being predicted as anomaly",scores)
